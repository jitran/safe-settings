...
`
  ...
  async removeOverrides (repoConfig, suborgRepoConfig) {
    const propertiesToCheck = [
      'protection.required_status_checks.contexts',
      'rulesets.rules.required_status_checks.required_status_checks',
    ]

    propertiesToCheck.forEach(property => {
      const keys = property.split('.')
      let obj = repoConfig
      let subObj = suborgRepoConfig
      ...
      if (obj[lastKey]) {
        if ((Array.isArray(obj[lastKey]) || typeof obj[lastKey] === 'string') && obj[lastKey].includes('{{EXTERNALLY_DEFINED}}')) {
          delete subObj[lastKey]
        } else  {
          for (const key in obj[lastKey]) {
            if (obj[lastKey][key].includes('{{EXTERNALLY_DEFINED}}')) {
              delete subObj[lastKey]
              return
            }
          }
  ...
  async updateRepos (repo) {
    ...
    if (subOrgConfig) {
      ...
      if (suborgRepoConfig) {
        ...
        removeOverrides(repoConfig, suborgRepoConfig)
    ...
    if (overrideRepoConfig) {
      removeOverrides(repoConfig, overrideRepoConfig)
